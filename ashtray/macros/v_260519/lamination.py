...
def lamination(lami_info: LamiInfo, dxf_settings: DxfSettings, step_obj, box):
    ...
    enum_faces_0 = add_enumurate_number(
        faces_0, dxf_settings.font_path, dxf_settings.text_height
    )
    enum_faces_1 = add_enumurate_number(
        faces_1, dxf_settings.font_path, dxf_settings.text_height
    )
...
def create_offset_faces(
    ...
    if is_even(sheets.sheet_num):
        base = interval / 2.0
        for i in range(0, sheets.sheet_num // 2 - 1):
...
def add_enumurate_number(faces_list, font, text_height):
    ...
    for num, face in enumerate(faces_list, start=1):
        ...
        text_shape.transformShape(
            App.Placement(
                App.Vector(-x_offset, -y_offset, 0), App.Rotation()
            ).toMatrix()
        )
        ...
        text_move_placement = App.Placement(
            App.Vector(com.x, com.y, com.z + 0.01), App.Rotation()
        )
        ...
        # text_placement = App.Placement(face.CenterOfMass, App.Rotation(0, 0, 0))
        # shape_string = Draft.make_shapestring(str(num), font_path, text_height)
        # shape_string.Placement = text_placement

        # ss.append(shape_string)
        # ss = Draft.make_shapestring(
        #     Text=str(num),
        #     FontFile=font_path,
        #     Size=text_height
        # )
        # ss.Label = f"Label_{num}_{face_string_id}"
        #
        # # 2. Attach the ShapeString natively to the 3D Face
        # ss.Support = [(target_obj, face_string_id)]
        # ss.MapMode = 'FlatFace'
