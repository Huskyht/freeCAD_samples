...
from toml_loader import toml_consts
...
def create_cube(selection):
    ...
    x_dim = math.ceil(bbox.XLength) + consts.cube_margin
    y_dim = math.ceil(bbox.YLength) + consts.cube_margin
    z_dim = consts.cube_total_height

    new_box = Part.makeBox(x_dim, y_dim, z_dim)
    placement = App.Placement(
        App.Vector(-x_dim / 2, -y_dim / 2, consts.cube_z_offset), App.Rotation()
    )
