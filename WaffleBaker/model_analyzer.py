...
try:
    ...
    subprocess.check_call([python_exe, "-m", "pip", "install", "rustworkx"])
...
class sheet_metal_graph:
    @staticmethod
    def build_graph_of_tangent_faces(shp: Part.Shape, root: int) -> rusx.Graph:
        # Created a simple undirected graph object.
        print(f"root_idx: {root}")
        graph_of_shape_faces = rusx.Graph()
        ...
        for c in rusx.connected_components(graph_of_shape_faces):
            if root in c:
                return graph_of_shape_faces.subgraph(c).copy()
        single_face_graph = rusx.Graph()
        single_face_graph.add_node(root)
        return single_face_graph

    @staticmethod
    def build_bfs_tree(graph, root):
        return rusx.bfs_tree(graph, source=root)

    @staticmethod
    def get_leaf_nodes(bfs_tree: rusx.DiGraph):
        leaf_nodes = []
    ...
    @staticmethod
    def nodes_path(bfs_tree: rusx.DiGraph, root_idx: int, leaf_idx: int):
        return rusx.shortest_path(bfs_tree, source=root_idx, target=leaf_idx)
...
class bend_analyzer:
    ...
    @staticmethod
    def separate_cyl_and_plane(bfs_tree, to_leaf_nodes):
        p = []
        c = []
        for idx in to_leaf_nodes:
            print(f"idx: {idx}")
            if bfs_tree[str(idx)]["type"] == "face":
                p.append(idx)

            elif bfs_tree[str(idx)]["type"] == "cylidnder":
                para_bend_line = False
                u0, u1, v0, v1 = bfs_tree[str(idx)]["data"].ParameterRange
                ...
                if len(c) >= 1:
                    ...
                    if is_parallel(
                        ...
                        bfs_tree[str(idx)]["data"].Surface.Axis,
                    ):
                        para_bend_line = True
                    else:
                        para_bend_line = False
    ...
    @staticmethod
    def get_planes_and_cylinders(bfs_tree: rusx.DiGraph, root_idx: int):
        planes = []
        cylinders = []
        ...
        for path in nodes_path:
            p, c = bend_analyzer.separate_cyl_and_plane(bfs_tree, path)
            planes.append(p)
            cylinders.append(c)
...
def main():
    ...
    # TEST:
    rusx.draw(bfs_tree, with_labels=True)
    plt.savefig(
        "bend_graph.png",
        format="png",
        dpi=300,
